...
function exibeOrganizadores(evento, URL) {
    let combo = "#txtOrganizadores";
    let loading = "#loading_organizadores";
    $(loading).fadeIn('slow');
    ...
    for (i = 0; i < vetor.length; i++) {
        if (vetor[i].selected) {
            ...
            $.post(URL + 'eventos/gravaOrganizadorAjax',
                ...
                function (data) {
                    if (data.length <= 0) {
                        alert('Impossível gralet organizador');
                    } //else {
                      //  location.reload();
                    //}
                });
...
function alteraControladorTable(URL, idOrganizador) {
    let idEvento = document.getElementById('txtId').value;
