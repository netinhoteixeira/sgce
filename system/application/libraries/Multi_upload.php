...
* Depende de um helper chamado replace_ascii.
...
class Multi_upload  {
	 ...
	 */	
	function go_upload($field = 'txtArquivo') {
            $CI =& get_instance();
            $CI->load->helper('replace_ascii');
...
//            echo "$fname\n\n<br><br>\n\n";            
            ...
            if ( ! is_uploaded_file(replaceAscii($_FILES[$field]['tmp_name'][$i])))
            {
            ...
            $CI->upload->file_temp = replaceAscii($_FILES[$field]['tmp_name'][$i]);
            $CI->upload->file_name = $CI->upload->_prep_filename(replaceAscii($_FILES[$field]['name'][$i]));
